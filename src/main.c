...
char notify_new[121] = HYBRID_OWNER;      /* Person to send a note to for new users */
...
/* For OSF/1 */
  ...
  if (((int) getuid() == 0) || ((int) geteuid() == 0))
    fatal("hybrid(core) will not run as root!", 0);
#endif

  /* secure hybrid(core) executable */
  chmod(argv[0], HYBRID_MODEX);
  ...
  cache_hit = 0;
  if (!pid_file[0])
    egg_snprintf(pid_file, sizeof pid_file, "%s.lock", botnetnick);
