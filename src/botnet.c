...
void answer_local_whom(int idx, int chan)
{
  ...
  dprintf(idx, format, "\002----------\002", "\002---------\002", "\002--------------------\002");
    ...
    if (dcc[i].type == &DCC_CHAT) {
      if ((chan == -1) || ((chan >= 0) && (dcc[i].u.chat->channel == chan))) {
        ...
        if (now - dcc[i].timeval > 300) {
          ...
          if (days > 0)
            sprintf(idle, " idle\002(\002%lud%luh\002)\002", days, hrs);
          else if (hrs > 0)
            sprintf(idle, " idle\002(\002%luh%lum\002)\002", hrs, mins);
          else
            sprintf(idle, " idle\002(\002%lum\002)\002", mins);
        } else
          idle[0] = 0;
        total++;
