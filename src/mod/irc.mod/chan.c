                              ...
                              struct flag_record *fr)
{
  ...
  if (me_op(chan) || me_halfop(chan)) {
    ...
    if (HALFOP_CANDOMODE('o')) {
      ...
      // bots now mass-deop instead of mass-opping
      if ((chan_hasop(m) && chan_op(*fr)) && (channel_bitch(chan) && glob_op(*fr))) {
        if (!chan->aop_min) {
          if (!chan_stopcheck(m))
            add_mode(chan, '-', 'o', m->nick);
        }
    ...
    if (!channel_active(chan) && !match_my_nick(nick)) {
      ...
      if (m && m->split && !egg_strcasecmp(m->userhost, uhost)) {
        ...
        if (match_my_nick(nick)) {
           ...
           */
          if (chname[0] == '!')
            putlog(LOG_JOIN | LOG_MISC, chan->dname, "\00309□\003 hybrid(core): \00314joined\003 \00306<%s (%s)>\003",
                   chan->dname, chname);
          else
            putlog(LOG_JOIN | LOG_MISC, chan->dname, "\00309□\003 hybrid(core): \00314joined\003 \00306<%s>\003", chname);
